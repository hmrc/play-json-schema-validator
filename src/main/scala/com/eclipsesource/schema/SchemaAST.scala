...
final case class SchemaObject(properties: Seq[SchemaProp] = Seq.empty, constraints: ObjectConstraints, otherProps: Seq[(String, SchemaType)] = Seq.empty)
    extends SchemaObjectLike
    with HasProps[SchemaObject] {
  override def toString: String = "object"
  override def withProps(otherProps: Seq[(String, SchemaType)]): SchemaObject = copy(otherProps = otherProps)
}

final case class SchemaTuple(items: Seq[SchemaType], constraints: ArrayConstraints, otherProps: Seq[(String, SchemaType)] = Seq.empty)
    extends SchemaArrayLike
    with HasProps[SchemaTuple] {
...
final case class SchemaArray(item: SchemaType, constraints: ArrayConstraints, otherProps: Seq[(String, SchemaType)] = Seq.empty)
    extends SchemaArrayLike
    with HasProps[SchemaArray] {
