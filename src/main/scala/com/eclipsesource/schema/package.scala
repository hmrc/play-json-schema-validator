...
package object schema {
  ...
  case class SchemaResolutionContext(refResolver: SchemaRefResolver, scope: SchemaResolutionScope, formats: Map[String, SchemaFormat] = DefaultFormats.formats) {
  ...
  implicit def noValidator[S <: SchemaType]: SchemaTypeValidator[S] = new SchemaTypeValidator[S] {
    override def validate(schema: S, json: => JsValue, resolutionContext: SchemaResolutionContext)(implicit lang: Lang): VA[JsValue] =
      Success(json)
  }
  ...
  implicit class SchemaTypeExtensionOps[S <: SchemaType](schemaType: S) {
    ...
    private def resolveRefAndValidate(json: JsValue, schemaRef: SchemaRef, context: SchemaResolutionContext)(implicit lang: Lang): VA[JsValue] = {

      context.refResolver.resolve(schemaRef, schemaRef.ref, context.scope) match {
        case -\/(error @ JsonValidationError(_, _*)) =>
          Results.failureWithPath(Keywords.Ref, error.message, context, json)
    ...
    private[schema] def doValidate(json: JsValue, context: SchemaResolutionContext)(implicit lang: Lang) = {
      (json, schemaType) match {
        ...
        case (_, SchemaValue(JsBoolean(false))) =>
          Results.failureWithPath("", ValidatorMessages("err.false.schema", schemaType, SchemaUtil.typeOfAsString(json)), context, json)

        case (_, SchemaValue(JsBoolean(true))) =>
          Success(json)

        case (_: JsObject, schemaObject: SchemaObject) =>
          schemaObject.validateConstraints(json, context)(objectValidator)

        case (_, schemaObject: SchemaObject) if schemaObject.constraints.any.schemaType.isEmpty =>
          schemaObject.validateConstraints(json, context)(objectValidator)

        case (_, c: CompoundSchemaType) =>
          c.validateConstraints(json, context)(compoundValidator)

        case (jsArray: JsArray, schemaArray: SchemaArray) =>
          schemaArray.validateConstraints(jsArray, context)(arrayValidator)

        case (jsArray: JsArray, schemaTuple: SchemaTuple) =>
          schemaTuple.validateConstraints(jsArray, context)(tupleValidator)

        case (jsNumber: JsNumber, schemaNumber: SchemaNumber) =>
          schemaNumber.validateConstraints(jsNumber, context)(numberValidator)

        case (jsNumber: JsNumber, schemaInteger: SchemaInteger) =>
          schemaInteger.validateConstraints(jsNumber, context)(integerValidator)

        case (jsBoolean: JsBoolean, schemaBoolean: SchemaBoolean) =>
          schemaBoolean.validateConstraints(jsBoolean, context)(booleanValidator)

        case (jsString: JsString, schemaString: SchemaString) =>
          schemaString.validateConstraints(jsString, context)(stringValidator)

        case (JsNull, schemaNull: SchemaNull) =>
          schemaNull.validateConstraints(json, context)(nullValidator)
        ...
        case (_, _) =>
          Results.failureWithPath(Keywords.Any.Type, ValidatorMessages("err.expected.type", schemaType, SchemaUtil.typeOfAsString(json)), context, json)
      }
    }
    ...
    private[schema] def validateConstraints(json: => JsValue, resolutionContext: SchemaResolutionContext)(implicit validator: SchemaTypeValidator[S], lang: Lang): VA[JsValue] = {
      schemaType.constraints match {
        case anyConstraint: HasAnyConstraint =>
          val r1 = validator.validate(schemaType, json, resolutionContext)
          val r2 = anyConstraint.any.validate(schemaType, json, resolutionContext)
          val r = Results.merge(
            r1,
            r2
          )
          r
        case _ =>
          val r = validator.validate(schemaType, json, resolutionContext)
          r
      }
