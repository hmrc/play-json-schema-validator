...
object ObjectValidators {
    ...
    ReaderWriterState { (context, status) =>
      ...
      val validated = properties.foldLeft(List.empty[(String, VA[JsValue])])((props, attr) =>
        json \ attr.name match {
          case _: JsUndefined =>
            if (required.contains(attr.name)) {
              attr.name ->
      ...
      val missing = required
        .filterNot(req => validated.exists(_._1 == req))
  ...
  def validatePatternProps(patternProps: Option[Map[String, SchemaType]], props: Props)(implicit lang: Lang): ValidationStep[Props] =
    ReaderWriterState { (context, status) =>
      ...
      val validated: Seq[(String, VA[JsValue])] = props.flatMap { prop =>
        {
          val matchedPatternProperties: Iterable[(String, SchemaType)] = patternProps
            .getOrElse(Seq.empty)
            .filter(pp => {
      ...
      val unmatchedProps = props.filterNot(prop => validatedProperties.contains(prop._1))
  ...
  def validateDependencies(schema: SchemaType, deps: Option[Map[String, SchemaType]], json: JsObject)(implicit lang: Lang): ValidationStep[Unit] = {
    ...
    def validatePropertyDependency(propName: String, dependencies: Seq[String], context: SchemaResolutionContext): VA[JsValue] = {
      ...
      val result = mandatoryProps.map(prop =>
        json.fields
          .find(_._1 == prop)
          .fold(
