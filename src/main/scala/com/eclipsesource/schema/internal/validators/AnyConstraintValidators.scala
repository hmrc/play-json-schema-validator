...
object AnyConstraintValidators {
  ...
  def validateIfThenElse(_if: Option[SchemaType], _then: Option[SchemaType], _else: Option[SchemaType])(implicit
      lang: Lang
  ): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] =
  ...
  def validateAnyOf(schema: SchemaType, anyOf: Option[Seq[SchemaType]])(implicit lang: Lang): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] = {
    ...
    @tailrec
    def untilFirstSuccess(json: JsValue, baseSchema: SchemaType, context: SchemaResolutionContext, schemas: List[SchemaType], results: List[VA[JsValue]]): List[VA[JsValue]] =
      schemas match {
        case s :: ss =>
  ...
  def validateEnum(enm: Option[Seq[JsValue]])(implicit lang: Lang): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] = {
    scalaz.Reader { context =>
      val enums = enm
      Rule.fromMapping { json =>
        enums match {
