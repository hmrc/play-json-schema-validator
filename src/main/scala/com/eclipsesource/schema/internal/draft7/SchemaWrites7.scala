...
trait SchemaWrites7 extends SchemaWrites { self: SchemaVersion =>
  ...
  lazy val anyConstraintWrites: OWrites[AnyConstraints] = {
    case AnyConstraints7(schemaTypeAsString, allOf, anyOf, oneOf, definitions, enm, const, not, desc, id, _if, _then, _else) =>
  ...
  lazy val arrayConstraintWrites: OWrites[ArrayConstraints] = { case ArrayConstraints7(maxItems, minItems, additionalItems, contains, unique, any) =>
  ...
  lazy val numberConstraintWrites: OWrites[NumberConstraints] = { case NumberConstraints7(min, max, multipleOf, format, any) =>
  ...
  lazy val stringConstraintWrites: OWrites[StringConstraints] = { case StringConstraints7(minLength, maxLength, pattern, format, any) =>
