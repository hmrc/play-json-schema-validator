...
trait SchemaReads7 extends SchemaReads { self: SchemaVersion =>
  ...
  override lazy val stringReads: Reads[SchemaString] =
    stringConstraintReads.flatMap(constraints => Reads.pure(SchemaString(constraints)))
  ...
  override lazy val anyConstraintReads: Reads[AnyConstraints] = {
    (
      ...
      val (schemaType, allOf, anyOf, oneOf, definitions, enm, const, not, desc, id, _if, _then, _else) = read
      AnyConstraints7(schemaType, allOf, anyOf, oneOf, definitions, enm, const, not, desc, id, _if, _then, _else)
    }
  }
  ...
  lazy val objectConstraintReads: Reads[ObjectConstraints] = {
    (
      ...
      val (patternProperties, additionalProperties, required, dependencies, minProperties, maxProperties, propertyNames, anyConstraints) = read
  ...
  lazy val stringConstraintReads: Reads[StringConstraints] = {
    (
      ...
      if (
        anyConstraints.schemaType.exists(_ != "string") ||
  ...
  lazy val numberConstraintReads: Reads[NumberConstraints] = {
    (
      ...
      if (
        typeAsString.exists(t => t != "number" && t != "integer")
