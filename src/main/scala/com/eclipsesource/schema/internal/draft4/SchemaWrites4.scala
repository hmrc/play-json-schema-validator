...
trait SchemaWrites4 extends SchemaWrites { self: SchemaVersion =>
  ...
  lazy val anyConstraintWrites: OWrites[AnyConstraints] = { case AnyConstraints4(schemaTypeAsString, allOf, anyOf, oneOf, definitions, enm, not, desc, id) =>
    asJsObject(Keywords.Any.Type, schemaTypeAsString) ++
      ...
      asJsObject(Keywords.Any.Enum, enm) ++
  ...
  lazy val arrayConstraintWrites: OWrites[ArrayConstraints] = { case ArrayConstraints4(maxItems, minItems, additionalItems, unique, any) =>
  ...
  lazy val stringConstraintWrites: OWrites[StringConstraints] = { case StringConstraints4(minLength, maxLength, pattern, format, any) =>
  ...
  lazy val numberConstraintWrites: OWrites[NumberConstraints] = { case NumberConstraints4(min, max, multipleOf, format, any) =>
    Json.obj()
    max.fold(emptyJsonObject)(max =>
      max.isExclusive match {
      ...
      min.fold(emptyJsonObject)(min =>
        min.isExclusive match {
      ...
      multipleOf.fold(emptyJsonObject)(multipleOf => Json.obj(Keywords.Number.MultipleOf -> multipleOf)) ++ anyConstraintWrites.writes(any) ++
      format.fold(emptyJsonObject)(formatName => Json.obj(Keywords.String.Format -> formatName))
  }
}
