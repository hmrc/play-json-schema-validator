...
trait SchemaReads4 extends SchemaReads { self: SchemaVersion =>
  ...
  override lazy val anyConstraintReads: Reads[AnyConstraints] = {
    (
      ...
      val (schemaType, allOf, anyOf, oneOf, definitions, enm, not, desc, id) = read
      AnyConstraints4(schemaType, allOf, anyOf, oneOf, definitions, enm, not, desc, id)
    }
  }
  ...
  lazy val objectConstraintReads: Reads[ObjectConstraints] = {
    (
      ...
      val (patternProperties, additionalProperties, required, dependencies, minProperties, maxProperties, anyConstraints) = read
