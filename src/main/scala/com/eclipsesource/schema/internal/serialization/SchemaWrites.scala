...
trait SchemaWrites { self: SchemaVersion =>
  ...
  implicit lazy val schemaTypeWriter: Writes[SchemaType] = Writes[SchemaType] {
  ...
  def additionalWrites: OWrites[SchemaType] = OWrites[SchemaType](other => throw new RuntimeException(s"Unknown schema type encountered: $other"))
