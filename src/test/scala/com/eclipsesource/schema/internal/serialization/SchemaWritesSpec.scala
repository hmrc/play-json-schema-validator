...
class SchemaWritesSpec extends Specification {
  ...
  "Schema Writes for draft 4" should {
    ...
    "write string with explicit type given" in {
      ...
      val schema: SchemaType = JsonSource.schemaFromString(stringConstraint).get
      Json.toJson(schema) must beEqualTo(
        Json.obj(
          "type" -> "string",
          "maxLength" -> 2
        )
    ...
    "write boolean" in {
      Json.toJson(SchemaBoolean(AnyConstraints4()).asInstanceOf[SchemaType]) must beEqualTo(Json.obj("type" -> "boolean"))
    }

    "write integer" in {
      Json.toJson(SchemaInteger(NumberConstraints4()).asInstanceOf[SchemaType]) must beEqualTo(Json.obj("type" -> "integer"))
    }
    ...
    "write object" in {
      Json.toJson(
        SchemaObject(Seq(SchemaProp("foo", SchemaNumber(NumberConstraints4()))), ObjectConstraints4()).asInstanceOf[SchemaType]
      ) must beEqualTo(
        Json.obj(
          "properties" -> Json.obj(
            "foo" -> Json.obj(
    ...
    "write root" in {
      Json.toJson(
        SchemaRoot(Some(Version4), SchemaNumber(NumberConstraints4())).asInstanceOf[SchemaType]
      ) must beEqualTo(
    ...
    "write array" in {
      Json.toJson(SchemaArray(SchemaNumber(NumberConstraints4()), ArrayConstraints4()).asInstanceOf[SchemaType]) must beEqualTo(
        Json.obj("items" -> Json.obj("type" -> "number"))
      )
    }

    "write tuple" in {
      Json.toJson(SchemaTuple(Seq(SchemaNumber(NumberConstraints4())), ArrayConstraints4()).asInstanceOf[SchemaType]) must beEqualTo(
    ...
    "write $ref" in {
      Json.toJson(
        SchemaRef(Ref("#"), AnyConstraints4()).asInstanceOf[SchemaType]
      ) must beEqualTo(Json.obj("$ref" -> "#"))
    }

    "write compound type" in {
      Json.toJson(CompoundSchemaType(Seq(SchemaNumber(NumberConstraints4()), SchemaBoolean(AnyConstraints4()))).asInstanceOf[SchemaType]) must beEqualTo(
        Json.obj(
    ...
    "write schema with definitions block" in {
      Json.toJson(
        SchemaNumber(
          NumberConstraints4().copy(any =
            AnyConstraints4(schemaType = Some("number"))
              .copy(definitions = Some(Map("foo" -> SchemaNumber(NumberConstraints4().copy(min = Some(Minimum(BigDecimal(3), Some(false))))))))
          )
        ).asInstanceOf[SchemaType]
      ) must beEqualTo(
        Json.obj(
    ...
    "write number with format" in {
      Json.toJson(
        SchemaInteger(NumberConstraints4().copy(format = Some("int32"))).asInstanceOf[SchemaType]
  ...
  "Schema Writes for draft 7" should {
    ...
    "write exclusiveMinimum" in {
      Json.toJson(SchemaInteger(NumberConstraints7(Some(Minimum(3, Some(true))))).asInstanceOf[SchemaType]) must beEqualTo(
